...
""" Testing suite for the PyTorch HyperMixer model. """


import unittest
from typing import Dict, List, Tuple

from transformers import HyperMixerConfig, is_torch_available
...
if is_torch_available():
    ...
    from transformers import (
        ...
        HyperMixerModel,
        HyperMixerTokenizerFast,
    )
    from transformers.models.hypermixer.modeling_hypermixer import (
        HYPER_MIXER_PRETRAINED_MODEL_ARCHIVE_LIST,
    )


# Override ConfigTester
class HyperMixerConfigTester(ConfigTester):
...
class HyperMixerModelTester:
    ...
    def create_and_check_model(self, config, input_ids, token_type_ids, sequence_labels, token_labels, choice_labels):
        model = HyperMixerModel(config=config)
    ...
    def create_and_check_for_pretraining(
        ...
        model = HyperMixerForPreTraining(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_masked_lm(
        ...
        model = HyperMixerForMaskedLM(config=config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_next_sentence_prediction(
        ...
        model = HyperMixerForNextSentencePrediction(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_question_answering(
        ...
        model = HyperMixerForQuestionAnswering(config=config)
        model.to(torch_device)
        model.eval()
        result = model(
            input_ids,
    ...
    def create_and_check_for_sequence_classification(
        ...
        model = HyperMixerForSequenceClassification(config)
        model.to(torch_device)
        model.eval()
    ...
    def create_and_check_for_token_classification(
        ...
        config.num_labels = self.num_labels
        model = HyperMixerForTokenClassification(config=config)
        model.to(torch_device)
        model.eval()
        result = model(input_ids, token_type_ids=token_type_ids, labels=token_labels)
    ...
    def create_and_check_for_multiple_choice(
        ...
        model = HyperMixerForMultipleChoice(config=config)
        model.to(torch_device)
        model.eval()
...
@require_torch
class HyperMixerModelTest(ModelTesterMixin, PipelineTesterMixin, unittest.TestCase):
    all_model_classes = (
        (
            HyperMixerModel,
            HyperMixerForPreTraining,
    ...
    pipeline_model_mapping = (
        {
            "feature-extraction": HyperMixerModel,
            "fill-mask": HyperMixerForMaskedLM,
            "question-answering": HyperMixerForQuestionAnswering,
            "text-classification": HyperMixerForSequenceClassification,
            "token-classification": HyperMixerForTokenClassification,
            "zero-shot": HyperMixerForSequenceClassification,
        }
        if is_torch_available()
        else {}
    )
    ...
    def setUp(self):
        self.model_tester = HyperMixerModelTester(self)
        self.config_tester = HyperMixerConfigTester(self, config_class=HyperMixerConfig, hidden_size=37)
    ...
    def test_model_from_pretrained(self):
        for model_name in HYPER_MIXER_PRETRAINED_MODEL_ARCHIVE_LIST[:1]:
            model = HyperMixerModel.from_pretrained(model_name)
            self.assertIsNotNone(model)


@require_torch
class HyperMixerModelIntegrationTest(unittest.TestCase):
    @slow
    def test_inference_for_masked_lm(self):
        ...
        model = HyperMixerForMaskedLM.from_pretrained("google/hypermixer-base")
        model.to(torch_device)
    ...
    def test_inference_long_sentence(self):
        model = HyperMixerForMaskedLM.from_pretrained("google/hypermixer-base")
        model.to(torch_device)
        tokenizer = HyperMixerTokenizerFast.from_pretrained("google/hypermixer-base")
    ...
    def test_inference_model(self):
        model = HyperMixerModel.from_pretrained("google/hypermixer-base")
        model.to(torch_device)
